...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root,edge_root, trainsize):
        ...
        # get filenames
        self.images = self.get_imagename(image_root)
        self.gts = self.get_imagename(gt_root)
        self.edges = self.get_imagename(edge_root)
    ...
    def __getitem__(self, index):
        ...
        # gt = randomPeper(gt)
        image, gt, edge = cv_random_flip(image, gt, edge) # 只用这个最好
    ...
    # get_imagename()为了提取消化道不同人的照片
    def get_imagename(self, root):
        image_persons = os.listdir(root)
        images = []
        for image_person in image_persons:
            image_names = os.listdir(os.path.join(root, image_person))
            for image_name in image_names:
                if image_name.endswith('.JPG') or image_name.endswith('.png') or image_name.endswith('jpg') or image_name.endswith('tif'):
                    image_path = os.path.join(root, image_person, image_name)
...
class test_dataset:
    ...
    def get_imagename(self, root):
        ...
        for image_person in image_persons:
            image_names = os.listdir(os.path.join(root, image_person))
            for image_name in image_names:
                if image_name.endswith('.JPG') or image_name.endswith('.png') or image_name.endswith('jpg') or image_name.endswith('bmp') or image_name.endswith('tif'):
                    image_path = os.path.join(root, image_person, image_name)
                    images.append(image_path)
        return images
